...
namespace dd::learn {
    ...
    void SetupTriangle() {
        ...
        res::LoadStbImage("resource/third_party/woodcrate.jpg", 4, std::addressof(texture0), std::addressof(width0), std::addressof(height0), std::addressof(channels0));
        res::LoadStbImage("resource/third_party/awesomeface.png", 0, std::addressof(texture1), std::addressof(width1), std::addressof(height1), std::addressof(channels1));
    ...
    void DrawTriangle(vk::CommandBuffer *command_buffer) {
        ...
        if (util::GetPointer(vk_texture_view0)->GetTexture()->GetImageLayout() == VK_IMAGE_LAYOUT_UNDEFINED) {
            ...
            util::GetReference(vk_image_memory).RelocateHostMemoryToDevice(command_buffer->GetCommandBuffer());
